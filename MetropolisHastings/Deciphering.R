...
# 1)
cppFunction('
  double objdens(NumericMatrix data,NumericMatrix ref, NumericVector theta){
    double lkh, logprior;
    int m=data.nrow();
    int n=theta.size()-2;
    //printf("n%d-m%d",n,m);
...
nsim <- 1000
init <- c(4,2,3,5,6)
MHBayes(20, init, objdens, proposal, X_train,y_train,.1)
mh.samp <- MHBayes(nsim, init, objdens, proposal, X_train,y_train, .1)
